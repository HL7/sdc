...
* item[0]
  * insert item(patient.id,http://hl7.org/fhir/Patient#Patient.id, (internal use\), #string)
  * readOnly = true
  * insert hidden()
  * insert initialExpression(%patient.id)
* item[+]
  * insert item(patient.birthDate, http://hl7.org/fhir/Patient#Patient.birthDate, Date of birth, #date)
  * required = true
  * insert initialExpression(%patient.birthDate)
* item[+]
  * insert item(patient.name,http://hl7.org/fhir/Patient#Patient.name,Name(s\),#group)
  * repeats = true
  * insert itemPopulationContext(patientName, #text/fhirpath, %patient.name)
  * item[0]
    * insert item(patient.name.family, http://hl7.org/fhir/Patient#Patient.name.family, Family name,#string)
    * required = true
    * insert initialExpression(%patientName.family)
  * item[+]
    * insert item(patient.name.given,http://hl7.org/fhir/Patient#Patient.name.given,Given name(s\),#string)
    * required = true
    * repeats = true
